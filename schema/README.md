...
show it as a JSON object (`"affiliated": {"NAME": "t4"}`) are the shape of that moment, not
today's - the current def in `org-node.schema.json` is the authority. Four statements no longer
describe the current corpus:
